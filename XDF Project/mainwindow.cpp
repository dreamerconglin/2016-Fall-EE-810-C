...
void MainWindow::on_actionImport_PDF_triggered()
{
    PDFview a(this);
    a.exec();
}

void MainWindow::on_actionImport_ePUB_triggered()
{
    QDialog a1(this);
    a1.resize(500,200);

    int size=5;
    std::wstring a[5]={L"哈哈1",L"哈哈2",L"haha3",L"haha哈哈4",L"ha嘿嘿ha5"};

    QString b[size];
    for(int i=0;i<size;i++){
        b[i]=QString::fromStdWString(a[i]);
    }

    QTableWidget table(&a1);
    table.resize(500,200);
    table.setEditTriggers(QAbstractItemView::NoEditTriggers);
    table.setRowCount(size);
    table.setColumnCount(1);
    QStringList horizontitle;
    horizontitle << "chapter name";
    table.setHorizontalHeaderLabels(horizontitle);
    table.horizontalHeader()->setDefaultAlignment(Qt::AlignCenter);
    for(int i=0;i<size;i++){
        table.setItem(i,0,new QTableWidgetItem(b[i]));
        //time1_9.setFlags(time1_9.flags() & (~Qt::ItemIsEditable));
    }
    //connect(tableWidget,SIGNAL(itemDoubleClicked(QTableWidgetItem*)),this,SLOT()));
    table.show();
    a1.exec();
}
